...
const FAQ = () => {
  ...
  const data = {
    ...
    rows: [
      ...
      {
        title: "Что означает кнопка «Не из базы» ?",
        content: (
          <p>
            <b>
              Данная кнопка позволяет выбрать 2 режима добавления данных в
              таблицу:
            </b>
            <br />
            <br />
            <b>1)</b> Режим выбора с «подкачкой» данных из базы - режим, при
            котором данные добавляется при выборе модели из базы автоматически
            <br />
            <b>2)</b> Ручной режим - режим при котором все данные записываются
            вручную
          </p>
        ),
      },
